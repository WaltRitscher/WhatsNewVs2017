...
namespace Models {
	public class ArtDataSource {
		...
		public static string GetArtPrice(string artName) {

			
			
			try
			{
				var q = from a in GetArtProjects()
								where a.ArtName.ToLower() == artName.ToLower()
								select a;

				var price = q.First().Price.ToString("r");
				return price;
			}
			catch (Exception ex)
			{
				 
				throw new ArgumentNullException ("This exception thrown for demo purposes", ex);
			}
